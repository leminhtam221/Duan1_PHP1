  ...
  <div class="row">
    <div class="col-md-8" style="padding: 0 15px;">
      ...
      <form method="POST" style="box-shadow: 0 10px 20px 0 rgba(0,0,0,0.07);padding: 15px 20px;border-radius: .25rem;">
        ...
        <button class="btn btn-primary bt-lg btn-block" type="submit" name="order" id="submitCheckout">Đặt Hàng</button>
      </form>
      <?php endif ?>
    </div>
    <div class="col-md-4">
...
$.getJSON("view/base/local.json", function(result) {
  ...
  var provincesHtml = [...provinces].map((item, i) =>
    `<option value="${item}">${(cities.includes(item.trim()))?'TP. ':''}${item}</option>`)
  ...
  $.getJSON("view/base/local.json", function(result) {
    $.each(result, function(i, field) {
      if (field.name == $('#province').val()) {
        ...
        var villagesHtml = [...villages].map((item) =>
          `<option value="${item}">${(Number(item)==item)?'Phường ':''}${item}</option>`)
